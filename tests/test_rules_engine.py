...
def create_rule_db(**kwargs):
    ...
    rule = Rule(
        ...
        definition_cel=kwargs.get("definition_cel", ""),
        timeframe=kwargs.get("timeframe", 600),
        timeunit=kwargs.get("timeunit", "seconds"),
        created_by=kwargs.get("created_by", "test"),
        creation_time=datetime.datetime.utcnow(),
        grouping_criteria=kwargs.get("grouping_criteria", []),
        resolve_on=kwargs.get("resolve_on", ResolveOn.NEVER.value),
        create_on=kwargs.get("create_on", CreateIncidentOn.ANY.value),
        require_approve=kwargs.get("require_approve", False),
        incident_name_template=kwargs.get("incident_name_template"),
        incident_prefix=kwargs.get("incident_prefix"),
...
def get_last_incidents(tenant_id, is_candidate=None, limit=10, offset=0, session=None, **kwargs):
    ...
    if is_candidate is False:
        query = query.where(Incident.is_visible == True)
    elif is_candidate is True:
        query = query.where(Incident.is_visible == False)
        
    total_count = s.exec(select(func.count()).select_from(query.subquery())).one()
...
@pytest.mark.skip(reason="Depends on specific API Gateway ID generation logic/endpoints")
