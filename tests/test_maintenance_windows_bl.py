...
import bl.maintenance_windows_bl
...
def test_strategy_clean_status(
    ...
    with (
        patch("core.db.db.existed_or_new_session", return_value=mock_session),
        patch(
...
def test_recover_strategy_closes_owned_session():
    ...
    with patch(
        "bl.maintenance_windows_bl.get_session_sync",
        return_value=tracking_session,
    ), patch(
        "bl.maintenance_windows_bl.get_maintenance_windows_started",
        return_value=[],
    ), patch(
        "bl.maintenance_windows_bl.get_alerts_by_status",
        return_value=[],
    ):
