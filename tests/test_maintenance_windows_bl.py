...
def update_maintenance_rule(*args, **kwargs): pass
...
def test_alert_with_missing_cel_field(
    mock_session, active_maintenance_window_rule, alert_dto
):
...
def test_strategy_clean_status(
    ...
    with (
        patch("src.core.db.db.existed_or_new_session", return_value=mock_session),
        patch(
            "src.bl.maintenance_windows_bl.get_last_alert_by_fingerprint",
            return_value=mock_last_alert,
        ),
        patch("src.bl.maintenance_windows_bl.get_alert_by_event_id", return_value=alert_maint),
    ):
...
def test_strategy_alert_block_by_window(
    ...
    with patch("src.core.db.db.existed_or_new_session", return_value=mock_session):
...
@pytest.mark.skip(reason="Depends on specific API Gateway schema models")
def test_strategy_alert_expired_by_current_time(
    ...
    update_maintenance_rule(
        ...
        rule_dto=MaintenanceRuleCreate(
            ...
            - 5,  # 10h - 5 seconds duration, so the end is just before current time
        ),
        authenticated_entity=MagicMock(
            tenant_id=SINGLE_TENANT_UUID, email="test@keephq.dev"
        ),
        session=db_session,
    )
    time.sleep(3)
...
@pytest.mark.skip(reason="Workflows are not executed in event-handler")
